...
class BodyNonbodyClassifier:
    ...
    def __init__(self):
        try:
            self.model = self.model = spacy.load(os.path.join(get_full_path(params['data']['path_to_noise_model']+'model-best')))
...
def test_code():
    ...
    output = cleaner.predict('Sir Keir Starmer has urged the government to invest urgently in jobs that benefit the environment. ' \
